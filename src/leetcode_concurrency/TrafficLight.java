...
public class TrafficLight {
    ...
    public void carArrived(int carId, int roadId, int direction, Runnable turnGreen, Runnable crossCar) {
        lock.lock();
        try {
            if ((roadId == 1 && !isRoadAGreen) || (roadId == 2 && isRoadAGreen)) {
                isRoadAGreen = roadId == 1;
                turnGreen.run();
            }
            crossCar.run();
        } finally {
    ...
    public static void main(String[] args) {
        ...
        new Thread(() -> trafficLight.carArrived(1, 1, 2,
                () -> System.out.println("Traffic Light On Road A Is Green"),
                () -> System.out.println("Car 1 Has Passed Road A In Direction 2"))).start();

        new Thread(() -> trafficLight.carArrived(2, 2, 4,
                () -> System.out.println("Traffic Light On Road B Is Green"),
                () -> System.out.println("Car 2 Has Passed Road B In Direction 4"))).start();

        new Thread(() -> trafficLight.carArrived(3, 2, 3,
                () -> System.out.println("Traffic Light On Road B Is Green"),
                () -> System.out.println("Car 3 Has Passed Road B In Direction 3"))).start();

        new Thread(() -> trafficLight.carArrived(4, 1, 1,
                () -> System.out.println("Traffic Light On Road A Is Green"),
                () -> System.out.println("Car 4 Has Passed Road A In Direction 1"))).start();
    }
}
